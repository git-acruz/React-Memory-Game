...
export default function EmojiButton({content, handleClick, selectedCardsEntry, matchedCardsEntry, index, emoji }) {
    ...
    return (
        <button className={`btn btn-emoji ${btnStyle}`} onClick={ selectedCardsEntry ? null : handleClick } disabled = { matchedCardsEntry } aria-label={ `Position $(index). ${btnAria}` } aria-live="polite">
            {btnContent}
        </button>
    )
}